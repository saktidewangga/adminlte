...
class Form extends \CI4Xpander\View\Component
{
    ...
    public function render()
    {
        ...
        if ($this->tabbed) {
            $view .= '<div class="nav-tabs-custom">';
            $tabNameList = [];
            $tabContent = [];
            $tabIndex = 0;
            foreach ($this->input as $tabName => $tab) {
                $tabNameList[] = '<li' . ($tabIndex == 0 ? ' class="active"' : '') . '><a href="#tab_' . str_replace(' ', '', $tabName) . '" data-toggle="tab" aria-expanded="' . ($tabIndex == 0 ? 'true' : 'false') .'">' . $tabName . '</a></li>';

                $theForm = '';
                foreach ($tab as $name => $input) {
                    $theForm .= $this->buildForm($name, $input, '', $method);
                }

                $tabContent[] = '<div class="tab-pane' . ($tabIndex == 0 ? ' active' : '') . '" id="tab_' . str_replace(' ', '', $tabName) . '">' . $theForm . '</div>';

                $tabIndex++;
            }
            $view .= '<ul class="nav nav-tabs">' . implode('', $tabNameList) . '</ul>';
            $view .= '<div class="tab-content">' . implode('', $tabContent) . '</div>';
            $view .= '</div>';
        } else {
            foreach ($this->input as $name => $input) {
                $view .= $this->buildForm($name, $input, '', $method);
            }
        }
    ...
    protected function buildForm($name, $input, $view, $method)
    {
        if (is_array($input) && array_key_exists('type', $input)) {
            ...
            if (in_array($input['type'], [
                Type::TEXT, Type::EMAIL, Type::PASSWORD, Type::DROPDOWN_AUTOCOMPLETE, Type::CHECKBOX, Type::CHECKBOX_SINGLE, Type::RADIO, Type::DATE, Type::DATE_RANGE, Type::TEXT_AREA, Type::WYSIWYG, Type::SLIDER, Type::SLIDER_RANGE, Type::TIME, Type::FILE, Type::DATATABLE_INPUT_TEXT, Type::TABLE, Type::CUSTOM_BUILDER,
            ])) {
            ...
            $value = isset($input[$valueName]) ? $input[$valueName] : (
                isset($submittedValue) ? $submittedValue : (
                    isset($input['default']) ? (
                        is_callable($input['default']) ? $input['default']() : $input['default']
                    ) : ''
                )
            );
            ...
            } elseif ($input['type'] == Type::DATATABLE_INPUT_TEXT) {
                // $rd = [];
                // foreach ($value['input']() as $d) {
                //     $rd[] = [
                //         'label' => $d->label,
                //         'input' => form_input("{$name}[{$d->name}]", '', array_merge($disabledAttr, [
                //             'class' => 'form-control',
                //             'autocomplete' => 'off'
                //         ]), 'text')
                //     ];
